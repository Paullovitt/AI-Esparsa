...
matrizes COO treinaveis. O modulo fornece o nucleo compartilhado pela base
textual V7.3 e pela arquitetura multimodal V8.
...
class ModeloGeradorEsparso(nn.Module):
    ...
    def _validar_tokens(self, tokens: Tensor) -> Tensor:
        if tokens.ndim != 2:
        ...
        if tokens.shape[1] > self.configuracao.maximo_contexto:
            return tokens[:, -self.configuracao.maximo_contexto :]
        return tokens

    def codificar_estados(self, tokens: Tensor) -> Tensor:
        """Executa o nucleo causal e devolve estados normalizados."""

        tokens = self._validar_tokens(tokens)
        ...
        return self.normalizacao_saida(estados)

    def projetar_logits(
        self,
        estados: Tensor,
        mascara: Tensor | None = None,
    ) -> Tensor:
        """Projeta todo o tempo ou somente posicoes supervisionadas."""

        if estados.ndim != 3:
            raise ValueError("estados deve possuir shape [lote, tempo, dimensao]")
        if mascara is not None:
            if mascara.dtype != torch.bool:
                raise TypeError("mascara de logits deve ser booleana")
            if mascara.shape != estados.shape[:2]:
                raise ValueError("mascara de logits possui shape incompativel")
            estados = estados[mascara]
        return F.linear(
            estados,
    ...
    def auxiliares_forward(self) -> dict[str, Tensor]:
        return {
            "gate_posicao": torch.sigmoid(self.gate_posicao),
        }

    def forward(self, tokens: Tensor) -> tuple[Tensor, dict[str, Tensor]]:
        estados = self.codificar_estados(tokens)
        return self.projetar_logits(estados), self.auxiliares_forward()
    ...
    def forward_alvos(
        ...
        """Evita a projecao vocabular em prompt e padding durante o treino."""

        estados = self.codificar_estados(tokens)
        return (
            self.projetar_logits(estados, mascara_alvos),
            self.auxiliares_forward(),
        )
