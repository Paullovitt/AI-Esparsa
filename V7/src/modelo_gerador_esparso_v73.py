...
class ModeloGeradorEsparsoV73(ModeloGeradorEsparsoCondicionalV73):
    ...
    def auditoria(self) -> dict[str, int | float | bool | str | dict]:
        ...
        caminho_incremental_fundido = (
            self.configuracao.dimensao == 128
            and self.configuracao.maximo_contexto <= 640
            and self.embedding.num_embeddings <= 512
        )
        auditoria.update(
            {
                ...
                "caminho_incremental_fundido": caminho_incremental_fundido,
                "lancamentos_kernel_por_token": (
                    11 if caminho_incremental_fundido else 0
                ),
                "motivo_incremental_nao_fundido": (
                    None
                    if caminho_incremental_fundido
                    else "requisitos estruturais ou vocabulario > 512"
                ),
                "memoria_escores_tempo_quadratica": bool(
                    self.configuracao_v73.usar_kernel_em_lotes
                ),
                "memoria_escores_tempo_quadratica_forward_lote": True,
